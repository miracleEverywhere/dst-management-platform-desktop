<template>
        ...
        <template #text>
            <v-data-table :headers="headers" :items="announceData" :loading="loading" hide-default-footer>
                ...
                <template v-slot:item.enable="{ item }">
                    <v-chip label :color="item.enable ? 'primary' : 'error'">
                        {{ item.enable ? "启用" : "禁用" }}
                    </v-chip>
                </template>
                ...
                <template v-slot:item.actions="{ item }">
                    <v-btn variant="tonal" size="small" class="mr-2" @click="openAnnounceUpdate(item)">更新</v-btn>
                    <v-btn variant="tonal" color="error" size="small" @click="handleAnnounceDelete(item)">删除</v-btn>
                </template>
            </v-data-table>
        </template>
    </v-card>
            ...
            <v-card-text>
                <v-sheet class="mx-auto pa-4">
                    <v-form ref="announceFormRef" :rules="announceFormRules">
                        <v-text-field v-model="announceForm.name" label="名称" class="mb-4"></v-text-field>
                        <v-switch v-model="announceForm.enable" :label="announceForm.enable ? '启用' : '禁用'" inset
                            class="mb-4"></v-switch>
                        <v-textarea v-model="announceForm.content" label="内容" class="mb-4"></v-textarea>
                        <v-text-field v-model="announceForm.frequency" label="频率" type="number"></v-text-field>
                        <div class="text-caption mt-2">每次执行的间隔，单位：秒</div>
                    </v-form>
                </v-sheet>
            </v-card-text>
            <v-card-actions>
    ...
    <v-dialog v-model="deleteDialogVisible" width="400">
        <v-card>
            ...
            <v-card-actions>
                <v-btn variant="plain" size="small" @click="cancelDelete">取消</v-btn>
                <v-btn color="primary" variant="tonal" size="small" @click="confirmDelete">确认</v-btn>
            </v-card-actions>
        </v-card>
    </v-dialog>
</template>
